...
class EditarDocenteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding?.let { binding ->
            ...
            binding.edtNombre.setText(docente.nombre)
    ...
    fun updateDocenteWS(docente : Docente) {
        ...
        val request = JsonObjectRequest(
            Request.Method.PUT, url.plus("/${docente.id}"), jsonBody,
            { response ->
                // Maneja la respuesta del servidor
                Log.d("POST Response", response.toString())
                showMessageAlert("Registro Exitoso", "El docente ${docente.nombre} se editó exitosamente", R.raw.success)
            },
            { error ->
